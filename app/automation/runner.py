...
class DSJFollowAutomation:
    ...
    async def _retry_step(self, step_func, step_name: str) -> bool:
        ...
        for attempt in range(MAX_RETRIES + 1):
            ...
            if attempt < MAX_RETRIES:
                await asyncio.sleep(RETRY_DELAY)
        logger.error(f"[{self.email}] Follow step '{step_name}' failed: {last_error}")
        return False
...
async def run_follow_order_for_account(
    ...
    try:
        ...
        automation_manager.running.pop(key, None)
